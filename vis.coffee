...
#     if typeof console.log == 'object' then log.apply.call(console.log, console, a) else console.log.apply(console, a)
#   return
...
if _? and s?
  ...
  Network = () ->
    ...
    network.updateSearch = (searchTerm) ->
      ...
      isSearching = false
      if !searchTerm or (searchTerm and searchTerm.length <= 0)
        this.clearStyle()
      else 
        node.each (d) ->
          element = d3.select(this)
    ...
    sortedArtists = (nodes,links) ->
      ...
      if sort == "links"
        ...
        artists = nodes.map((d) -> d.match).sort(d3.ascending)
        artists = artists.map (v) -> v.key
      else
    ...
    showDetails = (d,i) ->
      ...
      content += '<td style="text-align:right;"><p style="float:right;"><h4>Match Score</h4> ' + _.numberFormat(d.match,3) + '</p></td></tr>'    # content += '<hr class="tooltip-hr">'
      ...
      ol=[]
      link.each((l) -> if (l.source == d) then ol.push(l) )
      ol= _.sortBy(ol,'match')
      ul=_.take(ol,5);

      if ul and (ul.length-1) >0
        content += '</table></tbody>'
        ...
        ul.forEach (l) ->
          ...
          if (l.source==d and l.target==d)
           log('circular')
          else if (l.source == d)
            content += '<tr style="width:100%;"><td>'+_.truncate(l.target.name,22)+'</td><td style="float:right;"> '+_.numberFormat(l.target.match,3)+'</td></tr>' 
          else 
            content += '<tr style="width:100%;"><td>'+_.truncate(l.source.name,22)+'</td><td style="float:right;"> '+_.numberFormat(l.source.match,3)+'</td></tr>'
